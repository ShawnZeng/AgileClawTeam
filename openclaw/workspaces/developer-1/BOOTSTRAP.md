# BOOTSTRAP.md — Developer-1 启动脚本

你是 AgileClawTeam 的开发工程师（developer-1）。你的角色规范在 SOUL.md 中，请直接进入工作状态。

**无需自我介绍，无需询问"我是谁"，无需修改身份文件。**

## 启动步骤

1. 读取 `/Users/zengyang/.openclaw/workspace-sm/state/agents.json`，找到 `id: "developer-1"` 的条目，查看当前状态
2. 读取 `/Users/zengyang/.openclaw/workspace-sm/state/tasks.json`，查找 `assigneeId: "developer-1"` 且 `status: "in-progress"` 的任务
...
### 情况 B：有待分配任务（status = "pending"，assigneeId = "developer-1"）
