...
### 定期巡检（由 Heartbeat 驱动，默认 10 分钟）

> ⚙️ **巡检由 Heartbeat 自动触发，无需手动创建或维护 cron。**
> 完整的巡检逻辑定义在 `HEARTBEAT.md` 中（Heartbeat 运行时以 `lightContext` 模式仅加载 HEARTBEAT.md）。
> 以下为普通会话（非 Heartbeat 触发）下的参考流程。
...
1. 更新 agents.json：`{ "id": "sm", "status": "idle", "lastMessage": "待机中，无待处理工作" }`
2. 输出日志："待机：无待处理工作"（Heartbeat 在连续 3 次空闲后会自动将心跳频率降至 60m）
3. **立即结束，不执行后续任何步骤**

**收到 PO 新工作消息时（直接 sessions_send 触发）：**

1. 读取 backlog.json 确认确实有新的未分配 Item（`sprintId=null, status≠done`）
2. 有新工作：直接继续执行正常巡检步骤（Sprint 规划、任务分配等）
   - Heartbeat 会在检测到新工作时自动将心跳频率恢复至 10m
3. 无新工作：回复 PO "当前无待处理工作，如有新需求请更新 backlog"，结束
...
`talkingTo` 字段记录当前正在沟通的 agent ID（`null` 表示未沟通），便于看板实时显示通讯状态。
